...
private struct AssociatedKeys {
    ...
    static var SubtitleTimeObserver = "SubtitleTimeObserver"
}
...
extension ChiefsPlayer {
    ...
    var subtitlesTimeObserver: Any? {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.SubtitleTimeObserver) }
        set (value) { objc_setAssociatedObject(self, &AssociatedKeys.SubtitleTimeObserver, value, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
...
""")
    ...
    func removeCurrentSubtitles() {
        //Remove displayed subttile if exist
        if let timeObserver = subtitlesTimeObserver {
            player?.removeTimeObserver(timeObserver)
            subtitlesTimeObserver = nil
            parsedPayload = nil
    ...
    func addPeriodicNotification(parsedPayload: NSDictionary) {
        ...
        subtitlesTimeObserver = self.player?.addPeriodicTimeObserver(
    ...
    fileprivate func addSubtitleLabel() {
        ...
        guard let _ = subtitleLabel else {
            ...
            videoView?.insertSubview(subtitleLabel!, at: 1)
            ...
            videoView?.addConstraint(subtitleLabelHeightConstraint!)
